...
function OrderPost(){
    ...
    const onClickPost = () => {
        ...
        axios.post("/api/sellercenter/insertpostinfo", {
            RepostList: postList
        }).then((res) => {
            alert("배송정보가 등록되었습니다.");
    ...
    return(
        <DOV className="row">
            <div className="col-12">
                            ...
                            if(Istate === "배송준비중"){
                                return (
                                    <tr key={index}>
                                        ...
                                        <td><input type="text" onChange={(e) => onChangePostNum(e,index)} /></td>
                                        <td>{item["ID"]}</td>
                                        <td>{item["TIME"]}</td>
                                        <td>{Istate}</td>
                ...
                <div className="col-12">
                    <button className="btn btn-primary" onClick={onClickPost}>선택한 주문 배송처리</button>
                </div>
            </div>
        </DOV>
    )
